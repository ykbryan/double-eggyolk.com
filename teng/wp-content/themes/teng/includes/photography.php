...
$get_posts_db = "select id, post_author, post_date, post_content, post_title from wp_posts where id in (select object_id from wp_term_relationships where term_taxonomy_id = 4) and post_status = \"publish\"";
...
while ($info = mysql_fetch_array($get_posts)) {
	...
	while ($image = mysql_fetch_array($get_images)) {
		if($image[0] == "thumbnail")
			$portfolio .= '<img rel="#popup_portfolio" class="'.$image[0].'" src="'.$image[1].'"';
		else if($image[0] == "image")
			$portfolio .= 'alt="'.$image[1].'" title="'.$info['post_title'].'" desc="'.$info['post_content'].'" width="200">';
	}
